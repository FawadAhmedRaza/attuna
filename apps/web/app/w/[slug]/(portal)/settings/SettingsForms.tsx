...
// ─── Platform card (super_admin only) ───────────────────────────────
// Surfaces platform-level controls. Today these are mocked because we don't
// have a real super_admin user yet — when the dedicated `apps/admin` exists
// (per ARCHITECTURE.md), this whole card moves there. For now it lives behind
// `view_platform_admin` so the role-preview switcher in /roles can demo it.

const PLATFORM_KEY = "attuna_platform_v1";

const FEATURE_FLAGS = [
  {
    id: "brief_streaming",
    label: "Streaming briefs",
    description: "Render briefs token-by-token as Bedrock generates them.",
  },
  {
    id: "mobile_push",
    label: "Mobile push notifications",
    description: "Push to clients when therapist sends a suggestion. APNs/FCM only.",
  },
  {
    id: "shadow_pipeline",
    label: "Shadow pipeline (eval)",
    description: "Run new prompt versions in shadow against random 10% of briefs.",
  },
] as const;

const KILL_SWITCHES = [
  {
    id: "freeze_brief_generation",
    label: "Freeze brief generation",
    description: "Stops new Bedrock calls. Existing briefs stay readable.",
  },
  {
    id: "block_signups",
    label: "Block new signups",
    description: "Temporarily refuses new tenant onboarding.",
  },
] as const;
...
function loadPlatform(): PlatformShape {
  if (typeof window === "undefined") return {};
  try {
    return JSON.parse(localStorage.getItem(PLATFORM_KEY) ?? "{}") as PlatformShape;
  } catch {
    return {};
  }
}

function savePlatform(patch: PlatformShape) {
  try {
    const current = loadPlatform();
    localStorage.setItem(PLATFORM_KEY, JSON.stringify({ ...current, ...patch }));
  } catch {}
}
...
export function PlatformCard() {
  ...
  function toggleFlag(id: string, value: boolean) {
    const next = { ...flags, [id]: value };
    setFlags(next);
    savePlatform({ flags: next });
  }

  function toggleKill(id: string, value: boolean) {
    const next = { ...kill, [id]: value };
    setKill(next);
    savePlatform({ kill: next });
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      <SettingsCard
        ...
        description="Attuna staff only. These controls affect every tenant — change with care."
      >
        <div className="bg-warm-bg border-warm/30 mb-5 rounded-[12px] border px-4 py-3">
          <p className="text-warm-deep text-[12.5px] font-semibold">
            ⚠ Mock surface. Real changes require approval and audit log entry per HIPAA.md.
          </p>
        </div>

        <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--ink-mute)]">
          Feature flags
        </div>
      ...
      <SettingsCard
        ...
        description="Therapist credential submissions awaiting platform approval."
      >
        <div className="bg-bg-soft border-border-soft flex items-center justify-between rounded-[12px] border px-4 py-3">
          <div>
            <div className="text-ink text-[14px] font-semibold">3 awaiting review</div>
            <div className="text-ink-mute mt-0.5 text-[12px]">Oldest: 2 days ago</div>
          </div>
          <PillButton variant="outline" size="sm" disabled>
            Open queue
          </PillButton>
        </div>
        <p className="text-ink-mute mt-3 text-[12px]">
          Real queue lives in <code className="font-mono">apps/admin</code> when that scaffolding
          lands.
        </p>
      </SettingsCard>
      ...
      <SettingsCard
        title="Kill switches"
        description="Emergency-only controls. Each toggle writes to the audit log and pages the on-call."
      >
        <div className="flex flex-col gap-4">
          {KILL_SWITCHES.map((k) => (
            <Toggle
              ...
              checked={Boolean(kill[k.id])}
              disabled={!hydrated}
              onChange={(v) => toggleKill(k.id, v)}
            />
          ))}
        </div>
      </SettingsCard>
